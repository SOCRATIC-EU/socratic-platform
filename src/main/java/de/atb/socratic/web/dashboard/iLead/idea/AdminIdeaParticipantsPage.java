...
public class AdminIdeaParticipantsPage extends AdminIdeaPage {
    private static final long serialVersionUID = -5559578453943490669L;

    private final AdminIdeaParticipantsListPanel adminIdeaParticipantsListPanel;
    ...
    public AdminIdeaParticipantsPage(PageParameters parameters) {
        super(parameters);
        ...
        adminIdeaParticipantsListPanel = new AdminIdeaParticipantsListPanel("participantsList", Model.of(theIdea), feedbackPanel) {
        ...
        form.add(adminIdeaParticipantsListPanel.setOutputMarkupId(true));
    ...
    private void sendInvitations() throws NotificationException {
        ...
        if (StringUtils.isNotBlank(invitationEmails)) {
            ...
            invitationMailService.sendInvitationMessageFromIdea(
                    loggedInUser,
                    urlProvider.urlFor(RegisterPage.class, new PageParameters()),
                    urlProvider.urlFor(IdeaDetailsPage.class,
                            new PageParameters().set("ideaId", theIdea.getId()).set("id", theIdea.getCampaign().getId())), message,
                    theIdea.getShortText(), noRepeatedEmails.toArray(new String[noRepeatedEmails.size()]));
        }
    }
}
