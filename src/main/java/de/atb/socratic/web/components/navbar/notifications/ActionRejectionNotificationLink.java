 ...
 * @author ATB
 */
public class ActionRejectionNotificationLink extends NotificationBookmarkablePageLink<ActionRejectionNotification> {

    private static final long serialVersionUID = -3687349718251374845L;

    public ActionRejectionNotificationLink(String id, IModel<ActionRejectionNotification> model) {
        this(id, new PageParameters(), model);
    }

    public ActionRejectionNotificationLink(String id, PageParameters parameters, IModel<ActionRejectionNotification> model) {
        super(id, IdeaDetailsPage.class, new PageParameters().set("id", model.getObject().getIdea().getId()), model);
