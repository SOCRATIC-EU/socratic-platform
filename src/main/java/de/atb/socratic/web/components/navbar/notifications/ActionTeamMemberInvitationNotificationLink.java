 ...
 */
public class ActionTeamMemberInvitationNotificationLink extends
        NotificationBookmarkablePageLink<ActionTeamMemberInvitationNotification> {

    private static final long serialVersionUID = -3687349718251374845L;

    public ActionTeamMemberInvitationNotificationLink(String id, IModel<ActionTeamMemberInvitationNotification> model) {
        this(id, new PageParameters(), model);
    }

    public ActionTeamMemberInvitationNotificationLink(String id, PageParameters parameters,
                                                      IModel<ActionTeamMemberInvitationNotification> model) {
        super(id, ActionSolutionPage.class, new PageParameters().set("id", model.getObject().getAction().getId()), model);
        add(new Label("actionLeader", getAction().getPostedBy().getNickName())); // get action leader name
