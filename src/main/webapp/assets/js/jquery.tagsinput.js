...
(function($) {
	...
	var internal_counter = 0; // this is used to create unique ids for each
								// tag input
  ...
  $.fn.resetAutosize = function(options){
	...
	$.fn.tagExist = function(val) {	
			var settings = $(this).data('settings');
			var tagslist = $(this).val().split(settings.delimiter);	
			return (jQuery.inArray(val, tagslist) >= 0); // true when tag
															// exists, false
															// when not
	};
	...
	$.fn.tagsInput = function(options) { 
		...
		this.each(function() { 
			...
			// store the settings with the DOM object so we can use them in
			// other functions
			...
			if (settings.interactive) { 
				...
				// if the user clicks anywhere in the tag box, focus the input
				// field
				...
				if (settings.autocomplete) {
					// pass in the autocomplete settings to the autocomplete
					// plugin
					// provided by jquery UI
					$(data.fake_input).autocomplete(settings.autocomplete);
					
					// if an autocompleteselect handler has been passed in, use
					// that.
					...
					// track whether or not the autocomplete is open so we can
				...
				if (settings.typeahead) {
					// pass in the typeahead settings to the typeahead plugin
					// provided by twitter bootstrap
					$(data.fake_input).typeahead(settings.typeahead);
					$(data.fake_input).on('change', data, function(event) { 
						if ($(data.fake_input).val()!='' && $(data.fake_input).val() != event.data.defaultText) {
							if (!$(this).data('autocompleteopen')) { 
								if( (event.data.minChars <= $(data.fake_input).val().length) && (!event.data.maxChars || (event.data.maxChars >= $(data.fake_input).val().length)) )
									$(data.real_input).addTag($(data.fake_input).val(), { focus: true, unique: (settings.unique) });
							} else {
								// autocomplete is open, do not add tag because
								// this will be handled by the autocompleteselect
								// event.
							}
						}
						return false;
				...
				$(data.fake_input).on('blur',data,function(event) { 
					if ($(event.data.fake_input).val()!='' && $(event.data.fake_input).val()!=event.data.defaultText) {
						...
						$(event.data.fake_input).attr('placeholder', event.data.defaultText);
				...
				// Removes the not_valid class when user changes the value of
				// the fake input
